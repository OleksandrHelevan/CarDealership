...
namespace CarDealership.repo.impl
{
    public class ElectroCarRepositoryImpl : IElectroCarRepository
    {
        ...
        {
            // EF сам відфільтрує підтип ElectroCar через дискримінатор
        ...
        public IEnumerable<ElectroCar> GetFiltered(ElectroCarFilterDto filter)
        {
            ...
            if (filter.PriceFrom.HasValue)
                query = query.Where(c => (double)c.Price >= filter.PriceFrom.Value);

            if (filter.PriceTo.HasValue)
                query = query.Where(c => (double)c.Price <= filter.PriceTo.Value);
        ...
        {
            // CarType встановлюється автоматично EF через дискримінатор
        ...
        {
            var car = _context.Cars.OfType<ElectroCar>().FirstOrDefault(c => c.Id == id);
            if (car != null)
            {
